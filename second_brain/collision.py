...
class CollisionEngine:
    ...
    def index_high_novelty(self, insights: List[Insight]):
        """Index high-novelty insights into Memora for future recall."""
        ...
        for ins in insights:
            if ins.novelty >= threshold:
                try:
                    ...
                    vector_store.add(combined, metadata={
                        "source": "second_brain_collision",
                        "strategy": ins.strategy,
                        "novelty": ins.novelty,
                        "importance": 0.8,
                        "cross_layers": f"{ins.source_a}→{ins.source_b}",
                    })
                    logger.info("Indexed high-novelty insight (novelty=%d, %s→%s)",
                                ins.novelty, ins.source_a, ins.source_b)
                except Exception as e:
                    logger.warning("Failed to index insight: %s", e)
